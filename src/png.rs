...
impl Png {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        let mut png_contents = fs::read(path)?;

        Self::try_from(png_contents.as_ref())
    }
    ...
    pub fn remove_chunk(&mut self, chunk_type: &ChunkType) -> Result<Chunk> {
        self.chunks()
            ...
            .ok_or(Error::from("Chunk not found"))
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        if bytes.len() < 8 {
            return Err(Error::from("File too small to be a PNG"));
        }
        ...
        while i < raw_chunks.len() {
            ...
            i += chunk.as_bytes().len();
            chunks.push(chunk);
        }

        Ok(Png {
            header: [header[0], header[1], header[2], header[3], header[4], header[5], header[6], header[7]],
            chunks,
        })
    }
}
...
mod tests {
    ...
    fn test_chunk_by_type() {
        let png = testing_png();
        let chunk = png.chunk_by_type(&ChunkType {chunk: b"FrSt".to_owned()}).unwrap();
    ...
    fn test_append_chunk() {
        ...
        let chunk = png.chunk_by_type(&ChunkType {chunk: b"TeSt".to_owned()}).unwrap();
    ...
    fn test_remove_chunk() {
        ...
        png.remove_chunk(&ChunkType {
            chunk: b"TeSt".to_owned(),
        })
        .unwrap();
        let chunk = png.chunk_by_type(&ChunkType {chunk: b"TeSt".to_owned()});
